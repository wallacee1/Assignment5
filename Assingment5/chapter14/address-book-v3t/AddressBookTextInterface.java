...
public class AddressBookTextInterface
{
    ...
    private void remove()
    {
        try {
            ...
            System.out.print("Can't remove entry because key " + ex.getKey() + " is not in use !");
        } catch (Exception ex) {
    ...
    private void change()
    {
        System.out.print("Enter the key of entry you wish to change: ");
        String key = parser.readLine();
        ...
        try {
            ...
            System.out.print("Entry with key " + ex.getKey() + " doesn't exist!");
        } catch (Exception ex) {
            System.out.print(ex.getMessage());
    }
}
